...
class Strategy:
    ...
    def model_prediction_opt_posi_limit(cls, pred, amount):
        ...
        if pred=='Buy' or pred=='Sell':
            ...
            if Account.holding_side == pred and order_data['order_id'] == None:
                pass
            ...
            elif Account.holding_side != pred and order_data['order_id'] == None: #opposite pred but no order -> entry exit and entry order
                ad.add_action('entry', pred, bid if pred=='Buy' else ask, Account.holding_qty + amount, 'Limit', '', 'exit and entry order')
                ...
                print('holding size:', Account.holding_qty)
        else:
